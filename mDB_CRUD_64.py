#!/usr/bin/env python3
"""connect to a local mongoDB using pymongo 3.4.0
cause raspi only supports the 32-bit version
...
def connect_client():
    ...
    if LICENSE_STRING is None:
        stderr.write("""Connection refused.
...
def read_matchup(collection, field, searchstring):
    """reads all items in the collection that match
    a certain string and returns it as a dict"""
    if field == "_id":
...
def update_data(update_object, collection):
    ...
    # 32-bit version of mongodb doesn't allow manipulation of the _id tag
    object_id = update_object.pop("_id")
    return DB[collection].update_one(
        {"_id": object_id}, {"$set": update_object})
...
def main(argumentlist):
    """mongo adaptation to python using pymongo like the mongo shell.
    found out about it a little to late"""
    ...
    if (len(argumentlist) > 2) and (argumentlist[2] in commands):
        ...
        if argumentlist[2] == "list":
            if len(argumentlist) == 3:
                print(
                    [database["name"] for database in CLIENT.list_databases()])
