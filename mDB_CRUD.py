...
if __name__ == "__main__":
    usage_desc = """Usage:
    {0} <lic_file> <list> to list the present dbs.
    {0} <lic_file> <db_name> <status> to print the server status.
    {0} <lic_file> <db_name> <list> to list the tables.
    {0} <lic_file> <db_name> <read> <table> to read a tables content""".format(argv[0])
    if len(argv) > 2:
        LICENSE_STRING = ImportFromFile(argv[1]).import_direct()
        CLIENT, DB = db_init(argv[2])
        if len(argv) == 3:
            if argv[2] == "list":
